...
class MeasurePeriodValidatorTest < ActiveSupport::TestCase
  ...
  test 'Should return error for HQR not aligned to quarter' do
    validator = CypressValidationUtility::Validate::MeasurePeriodValidator.new('HQR_EPM_VOL', 'eh', '2017', 'cat1_r5')
    change_reporting_period(@document, '20170101000000', '20171231000000')
    errors = validator.validate(@document)

    assert_equal 1, errors.count, "Expected 1 error, got #{errors}"
    msg = 'Reported Measurement Period (20170101000000 - 20171231000000) does not align to a quarter (ex, 1/1-3/31, 4/1-6/30, 7/1-9/30, 10/1-12/31).'
    assert_equal msg, errors[0].message
  end

  test 'Should not return error for HQR aligned to quarter' do
    validator = CypressValidationUtility::Validate::MeasurePeriodValidator.new('HQR_EPM_VOL', 'eh', '2017', 'cat1_r5')
    change_reporting_period(@document, '20170101000000', '20170331000000')
    errors = validator.validate(@document)
  ...
  test 'Should return error for PQRS not aligned to year' do
    validator = CypressValidationUtility::Validate::MeasurePeriodValidator.new('CEC', 'ep', '2017', 'cat1_r5')
    change_reporting_period(@document, '20170103000000', '20171230000000')
    errors = validator.validate(@document)

    assert_equal 2, errors.count, "Expected 2 errors, got #{errors}"

    msg = 'Reported Measurement Period should start on 20170101'
    assert_equal msg, errors[0].message

    msg = 'Reported Measurement Period should end on 20171231'
  ...
  test 'Should not return error for PQRS not aligned to year with MIPS program' do
    validator = CypressValidationUtility::Validate::MeasurePeriodValidator.new('MIPS_INDIV', 'ep', '2017', 'cat1_r5')
    change_reporting_period(@document, '20170103000000', '20171230000000')
    errors = validator.validate(@document)
  ...
  test 'Should return error for PQRS not aligned to year with MIPS program but not at least a day of data' do
    validator = CypressValidationUtility::Validate::MeasurePeriodValidator.new('MIPS_INDIV', 'ep', '2017', 'cat1_r5')
    change_reporting_period(@document, '20170103000000', '20170102000000')
    errors = validator.validate(@document)
  ...
  test 'Should return error for no encounters during rp' do
    validator = CypressValidationUtility::Validate::MeasurePeriodValidator.new('none', 'none', 'none', 'cat1_r5')
